...
def summarize_comment(url: str, chunk: str, summary_length: int) -> str:
    message = f"Vui lòng cung cấp bản tóm tắt cực kỳ dài và toàn diện dựa trên các câu đánh giá của video {url}, với các đánh giá sau: {chunk} \n\n Vui lòng đảm bảo độ dài tóm tắt xấp xỉ {summary_length} từ."
